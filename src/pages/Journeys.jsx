...
export default function Journeys() {
  ...
  const recommendations = journeys.filter((journey) => journey.feelings.includes(feeling))
    ...
    return (
      <AppLayout className="message-screen">
        ...
        <div className="statement">
          ...
          <p>A journey for this feeling<br />is on its way.</p>
        </div>
  ...
  return (
    <AppLayout className="journeys-screen">
      ...
      <div className="journey-recommendation">
        <p className="journey-title">{primary.title} · {primary.durationLabel}</p>
        <p className="body-copy">{primary.instruments.join(' + ')}</p>
      </div>
      <span className="down-arrow" aria-hidden="true">↓</span>
      <PrimaryAction to={`/journey/${primary.slug}`}>BEGIN JOURNEY</PrimaryAction>
      {others.length > 0 && (
